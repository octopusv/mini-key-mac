...
fn find_interface_and_endpoint(
    ...
    for iface_num in interface_nums {
        ...
        // Log all endpoints for diagnostics
        for ep in intf_desc.endpoint_descriptors() {
            debug!(
                "  iface {} class={:#04x} sub={:#04x} proto={:#04x} endpoint addr={:#04x} type={:?} dir={:?}",
                iface_num,
                intf_desc.class_code(),
                intf_desc.sub_class_code(),
                intf_desc.protocol_code(),
                ep.address(),
                ep.transfer_type(),
                ep.direction()
            );
        ...
        // Only consider the vendor config interface (HID, no boot protocol).
        if !(intf_desc.class_code() == 0x03
            && intf_desc.sub_class_code() == 0x00
            && intf_desc.protocol_code() == 0x00)
        {
            debug!("skipping interface with unexpected parameters: {intf_desc:#?}");
            continue;
        }

        // Prefer the requested endpoint address, but fall back to the first
        // interrupt OUT endpoint on this interface. Endpoint numbers vary by model
        // (e.g. 0x514c:8851 uses 0x02 instead of the usual 0x04).
        let endpt = intf_desc
            .endpoint_descriptors()
            .find(|ep| ep.transfer_type() == TransferType::Interrupt && ep.address() == endpoint_addr)
            .or_else(|| {
                intf_desc.endpoint_descriptors().find(|ep| {
                    ep.transfer_type() == TransferType::Interrupt
                        && ep.direction() == Direction::Out
                })
            });

        if let Some(endpt_desc) = endpt {
