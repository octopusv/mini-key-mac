...
const SUPPORTED_DEVICES: &[(KeyboardModel, u16, u16)] = &[
    ...
    // 3 keys + 1 knob variant (3KV1 / "MINI KeyBoard.exe").
    // Despite the 0x514c vendor id, this model speaks the classic k884x (0xfe)
    // protocol, not the 0x8850 (0xfd) one. Verified empirically: buttons -> id 1..3,
    // knob ccw/press/cw -> id 16/17/18, OUT endpoint 0x02.
